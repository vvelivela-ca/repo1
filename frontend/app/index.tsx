...
export default function Dashboard() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (holdingsData.length > 0) {
        const symbols = holdingsData.map((h) => h.symbol).join(',');
  ...
  const getTotalCost = () => {
    return holdings.reduce((sum, h) => sum + h.avg_price * h.shares, 0);
  };
  ...
  const getTotalDayChange = () => {
    return holdings.reduce((sum, h) => {
      ...
      if (!q) return sum;
      return sum + (q.price - q.previous_close) * h.shares;
    }, 0);
  };
  ...
  const renderHolding = ({ item }: { item: Holding }) => {
    ...
    const dayChg = (price - prevClose) * item.shares;
      ...
      <TouchableOpacity
        ...
        onPress={() =>
          router.push({
            ...
            params: { symbol: item.symbol, holdingId: item.id },
          })
        }
      >
        <View style={styles.holdingTop}>
          ...
          <View style={styles.holdingRight}>
            ...
            <View style={[styles.changeBadge, isDayPositive ? styles.greenBg : styles.redBg]}>
              ...
              />
              <Text style={[styles.changeText, isDayPositive ? styles.greenText : styles.redText]}>
                {formatPct(dayChgPct)}
              </Text>
            </View>
          </View>
              ...
              <View style={styles.summaryRow}>
                <View style={styles.summaryChip}>
                  <Text style={[styles.summaryChipText, totalGain >= 0 ? styles.greenText : styles.redText]}>
                    {totalGain >= 0 ? '+' : ''}
                    {formatCurrency(totalGain)} ({formatPct(totalGainPct)})
                  </Text>
                ...
                <View style={styles.summaryChip}>
                  <Text style={[styles.summaryChipText, dayChange >= 0 ? styles.greenText : styles.redText]}>
                    {dayChange >= 0 ? '+' : ''}
                    {formatCurrency(dayChange)} ({formatPct(dayChangePct)})
                  </Text>
