...
interface Holding { id: string; symbol: string; shares: number; avg_price: number; portfolio_id: string; currency: string; asset_type: string; exchange: string | null; notes: string | null; }
...
export default function Dashboard() {
  ...
  type SortKey = 'value' | 'day' | 'gain' | 'return' | 'symbol';
  ...
  const sortedHoldings = [...holdings].sort((a, b) => {
    ...
    if (sortKey === 'symbol') {
      ...
      const va = convert((qa?.price || 0) * a.shares, qa?.quote_currency || 'USD');
      const vb = convert((qb?.price || 0) * b.shares, qb?.quote_currency || 'USD');
      diff = va - vb;
    } else if (sortKey === 'day') {
      ...
      const ga = convert((qa?.price || 0) * a.shares, qa?.quote_currency || 'USD') - convert(a.avg_price * a.shares, a.currency || 'USD');
      const gb = convert((qb?.price || 0) * b.shares, qb?.quote_currency || 'USD') - convert(b.avg_price * b.shares, b.currency || 'USD');
      diff = ga - gb;
      ...
      const ra = costA > 0 ? ((convert((qa?.price || 0) * a.shares, qa?.quote_currency || 'USD') - costA) / costA) * 100 : 0;
      const rb = costB > 0 ? ((convert((qb?.price || 0) * b.shares, qb?.quote_currency || 'USD') - costB) / costB) * 100 : 0;
      diff = ra - rb;
    }
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <FlatList testID="holdings-list" data={sortedHoldings} keyExtractor={(item) => item.id} renderItem={renderHolding}
          ...
          <View>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.sortScroll} contentContainerStyle={styles.sortContainer}>
              {SORT_OPTIONS.map((opt) => {
                ...
                return (
                  <TouchableOpacity key={opt.key} onPress={() => cycleSort(opt.key)}
                    style={[styles.sortChip, active && styles.sortChipActive]} activeOpacity={0.7}>
